...
class StageEditor(ttk.Frame):
    ...
    def bulk_add_enemy(self):
        ...
        # どのキーを等差的に変化させるか？
        # 例: "pos.x" / "spawnTime" / "" (空なら何もしない)
        key_path = simpledialog.askstring(
            "Bulk Add",
            "等差的に変化させるキーを入力してください\n"
            "例: pos.x, pos.y, spawnTime\n"
            "空のままOKで、単純クローンになります。"
        )
        if key_path is None:
            # キャンセル
        ...
        if use_arithmetic:
            # テンプレートから元の値を取得
            base_value = self._get_value_by_path(template, key_path)
            if not isinstance(base_value, (int, float)):
                messagebox.showerror(
                    "Bulk Add",
                    f"キー '{key_path}' の元の値が数値ではありません。\n"
                    f"現在の値: {base_value!r}"
                )
                return

            # 増分を取得
            step_str = simpledialog.askstring(
                "Bulk Add",
                f"キー '{key_path}' の1個ごとの増分を入力してください\n"
                f"例: 20, 5, -30 など"
            )
            if step_str is None:
                return
            ...
            try:
                # int/float を受け取れるように float でパース
                step_value = float(step_str)
            except ValueError:
                messagebox.showerror(
                    "Bulk Add",
                    f"増分 '{step_str}' を数値として解釈できません。"
                )
                return
        ...
        for i in range(n):
            ...
            # 等差的な変更
            if use_arithmetic:
                # i=0 のとき base + step, i=1 で base + 2*step ... にする
                new_val = base_value + step_value * (i + 1)
                # intにしたい場合、元がintならintにキャストしておく
                if isinstance(base_value, int):
                    new_val = int(new_val)
                self._set_value_by_path(new_enemy, key_path, new_val)
